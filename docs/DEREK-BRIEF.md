...
| Grade spread | A 17 · B 26 · C 4 · F 5 (continuous 8–93, 26 distinct scores) |
| **Web-services leads** | **6** |
| **Recruitment leads** | **5** (1 with a confirmed live role) |

### Web-services leads (→ Crush) — 6
Independent operators with missing, broken, or weak websites:
- **Zeina's Office Space**, **KOWORK (Airdrie)** — no website at all
- **The Collective 12|12**, **The Collab Hub**, **Highfield Hive** — domains no longer resolve (sites offline)
- **Work Buddiz** — live but thin (score 57)
...
2. **Intelligence** (`/intelligence`) — the money view: the full Calgary market, scored, with the **6 web-services leads** and **5 recruitment leads** tables (TradeSpace's live opening called out) + the honest methodology. *This is the result.*
3. **Recruitment** (`/recruitment`) — the Calgary talent leads, then scroll to the **global Radar** (47 live openings) — "the same engine pointed at your actual market." Hit **Request talent** to show the capture form works.
...
> "This is one scan of one market — it handed us 6 web-services leads and 5 recruitment leads with sources, plus 47 live leadership openings across the major operators. Do you want Bottle Rocket pointed at this, and which market do we scan next?"
...
Working, verified, on branch `calgary-real-rebuild` (pushed to origin). Build clean; every reachable page console-clean; both lead-capture forms work end-to-end (file-based, no DB). Demo runs locally with a dummy `.env.local`; a real launch needs real DB/auth/email provisioning. See `docs/SPRINTS.md` for the full build trail.

*Built by Crush Digital Atelier · Calgary scan 2026-06-21, re-scored 2026-06-25, Radar refreshed 2026-06-22 · reproducible, not mocked up.*
